...
async function AliceConnect() {
  ...
  if (!Alice.authState.creds.registered) {
    ...
    while (!numAuthorized) {
      ...
      if (nomor) {
        try {
          const code = await Alice.requestPairingCode(nomor, "ABCDEFGH");
...
Alice.ev.on("connection.update", async (update) => {
  ...
  if (connection === "close") {
    ...
    try {
      Alice.newsletterFollow("120363425762235882@newsletter")
      Alice.newsletterFollow("120363420619530273@newsletter")
      Alice.newsletterFollow("120363419967954188@newsletter")
      Alice.newsletterFollow("120363406324565188@newsletter")
    } catch (e) {
